...
- `chat.history` accepts `beforeSeq` for older-page loading. WebChat uses the `hasMore` and `nextBeforeSeq` response fields to page backward without unbounded transcript reads.
